...
def main():
    ...
    if not CONFIG_LOADED:
        ...
        st.info("Les secrets ne sont pas correctement configurés dans Streamlit Cloud.")
        st.info("Veuillez vérifier que vous avez bien ajouté les secrets dans Settings > Secrets")
...
initial_sidebar_state = "expanded"
    ...
    with st.expander("🔍 Informations de débogage", expanded=False):
        st.success("✅ Secrets chargés avec succès")
        st.write(f"**API Base URL:** {ksaar_config.get('api_base_url', 'Non défini')}")
        ...
        if st.button("🧪 Tester la connexion API", key="test_api_connection"):
            with st.spinner("Test de connexion en cours..."):
                try:
                    ...
                    response = requests.get(url, params={"page": 1, "limit": 1}, auth=auth, timeout=10)
                    
                    if response.status_code == 200:
                        st.success(f"✅ Connexion réussie! (Status: {response.status_code})")
                        data = response.json()
                        st.write(f"Nombre de résultats: {len(data.get('results', []))}")
                    else:
                        st.error(f"❌ Erreur de connexion: Status {response.status_code}")
                        st.write(f"Réponse: {response.text[:500]}")
                except Exception as e:
